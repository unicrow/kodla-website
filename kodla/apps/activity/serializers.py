...
class PrevActivitySerializer(ActivityListSerializer):
    ...
    class Meta:
        model = Activity
        fields = ('description', 'year', 'sponsors', 'speakers', 'gallery')
...
class ActivityRetrieveSerializer(ActivitySerializer):
    ...
    prev_activities = serializers.SerializerMethodField()
    ...
    class Meta:
        ...
        fields = (
            ...
            'social_accounts', 'sponsors', 'programs', 'speakers', 'maps', 'questions', 'prev_activities'
        )
    
    def get_prev_activities(self, obj):
        prev_serializer = PrevActivitySerializer(Activity.objects.exclude(id=obj.id).order_by("year"), context=self.context, many=True)

        return prev_serializer.data
